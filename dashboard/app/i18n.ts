...
export type WorkspaceTranslation = {
  ...
  artifactTypes: Readonly<Record<'file' | 'directory' | 'document' | 'source_code' | 'configuration' | 'unknown', string>>;
  noProjects: string;
...
export type Translation = {
  ...
  phases: readonly [PhaseTranslation, PhaseTranslation, PhaseTranslation, PhaseTranslation, PhaseTranslation, PhaseTranslation, PhaseTranslation, PhaseTranslation, PhaseTranslation, PhaseTranslation];
...
export const translations: Record<Language, Translation> = {
  zh: {
    ...
    localOnly: '仅限本地', dataStays: '你的数据只保存在这台设备上', workspace: '项目工作区', greeting: '项目概览', planComplete: '项目建设计划 1.6 · 已完成',
    ...
    phases: [
      ...
      { name: '项目详情', summary: '从识别结果进入单项目检查详情。', features: ['点击项目名称打开详情', '查看检查状态、技术栈与资产分类', '清楚标记受限项目且不虚构统计'] },
    ...
    milestones: [['可点击的本机项目详情', '从识别结果查看检查状态、技术栈与资产分类'], ['超大项目不中断目录识别', '超过检查范围的项目明确标记为受限，其他项目继续识别'], ['本地项目目录管理', '选择扫描目录，识别项目并按设定间隔检查变化'], ['明确的项目组成与发布记录', '区分项目组成、项目关系与实时分析']],
  ...
  en: {
    ...
    localOnly: 'Local only', dataStays: 'Your data stays on this device', workspace: 'Project workspace', greeting: 'Project overview', planComplete: 'Project Plan 1.6 · Complete',
    ...
    workspaceManager: { section: 'Project discovery', title: 'Choose folders to manage', description: 'Choose a local folder and Project Atlas will identify the software projects inside it. Automatic checks start only when you enable them.', chooseDirectory: 'Choose folder', selecting: 'Waiting for selection…', connecting: 'Connecting to the local service…', serviceUnavailable: 'Local service is not running', serviceHelp: 'Start the Project Atlas local service, then reconnect.', retry: 'Reconnect', emptyTitle: 'No project folders added', emptyDescription: 'Choosing a folder runs the first project discovery immediately.', initialScanComplete: 'Folder added and initial project discovery completed.', scanComplete: 'Project check completed.', operationFailed: 'The operation failed.', scanDirectory: 'Scan folder', lastChecked: 'Last checked', neverScanned: 'Not checked yet', scanNow: 'Check now', remove: 'Remove', confirmRemove: 'Remove this folder and its local check records? Files in the folder will not be deleted.', automaticChecks: 'Automatic checks', interval: 'Check interval', minutes: 'minutes', latestChanges: 'Latest check result', projectCount: 'projects', newProjects: 'new', changedProjects: 'changed', removedProjects: 'removed', limitedProjects: 'limited', discoveredProjects: 'Discovered projects', openProject: 'View project', projectDetail: 'Project details', closeDetail: 'Close project details', projectPath: 'Project path', checkResult: 'Latest check result', structureCheck: 'Structure check', completeCheck: 'Complete', limitedCheck: 'Limited', technologyStack: 'Technology stack', assetOverview: 'Project asset overview', limitedDetail: 'This project exceeds the current structure check limit. Project Atlas identified it but will not show incomplete asset statistics.', detailBoundary: 'This view uses the latest local check result. It is not AI analysis.', artifactTypes: { file: 'Files', directory: 'Directories', document: 'Documents', source_code: 'Source code', configuration: 'Configuration', unknown: 'Unclassified' }, noProjects: 'No software projects were identified in this folder.', unknownTechnology: 'Technology not identified', items: 'assets', analysisLimited: 'Project size exceeds the current check scope', assetCountUnavailable: 'Asset count unavailable', changeStatus: { added: 'New', changed: 'Changed', unchanged: 'Unchanged', recorded: 'Recorded' }, localBoundary: 'Folder paths, project information, and check records stay on this device. Automatic checks stop when the local service is closed.' },
    executionPlan: 'Project development', architectureEvolution: 'Development progress', phasesComplete: '10 / 10 stages',
    phases: [
      ...
      { name: 'Project details', summary: 'Open single-project check details from discovery results.', features: ['Open details by clicking a project name', 'Review check status, technologies, and asset categories', 'Clearly mark limited projects without invented statistics'] },
    ...
    milestones: [['Clickable local project details', 'Review check status, technologies, and asset categories from discovery results'], ['Oversized projects no longer stop folder discovery', 'Projects beyond the check scope are marked as limited while other projects continue'], ['Local project folder management', 'Choose scan folders, identify projects, and check changes on schedule'], ['Clear project composition and release records', 'Distinguishes composition, project relationships, and real-time analysis']],
  ...
  ru: {
    ...
    localOnly: 'Только локально', dataStays: 'Ваши данные остаются на этом устройстве', workspace: 'Рабочее пространство', greeting: 'Обзор проекта', planComplete: 'План проекта 1.6 · Завершён',
    ...
    workspaceManager: { section: 'Обнаружение проектов', title: 'Выберите папки для управления', description: 'Выберите локальную папку, и Project Atlas найдёт в ней программные проекты. Автоматические проверки включаются только вами.', chooseDirectory: 'Выбрать папку', selecting: 'Ожидание выбора…', connecting: 'Подключение к локальной службе…', serviceUnavailable: 'Локальная служба не запущена', serviceHelp: 'Запустите локальную службу Project Atlas и повторите подключение.', retry: 'Подключиться снова', emptyTitle: 'Папки проектов не добавлены', emptyDescription: 'Первый поиск проектов начнётся сразу после выбора папки.', initialScanComplete: 'Папка добавлена, первый поиск завершён.', scanComplete: 'Проверка проектов завершена.', operationFailed: 'Операция не выполнена.', scanDirectory: 'Папка сканирования', lastChecked: 'Последняя проверка', neverScanned: 'Ещё не проверялась', scanNow: 'Проверить', remove: 'Удалить', confirmRemove: 'Удалить эту папку и её локальные записи? Файлы в папке не будут удалены.', automaticChecks: 'Автоматические проверки', interval: 'Интервал', minutes: 'минут', latestChanges: 'Результат последней проверки', projectCount: 'проектов', newProjects: 'новых', changedProjects: 'изменено', removedProjects: 'удалено', limitedProjects: 'ограничено', discoveredProjects: 'Найденные проекты', openProject: 'Открыть проект', projectDetail: 'Сведения о проекте', closeDetail: 'Закрыть сведения', projectPath: 'Путь проекта', checkResult: 'Последний результат', structureCheck: 'Проверка структуры', completeCheck: 'Полная', limitedCheck: 'Ограниченная', technologyStack: 'Технологии', assetOverview: 'Обзор объектов проекта', limitedDetail: 'Проект превышает текущий предел проверки структуры. Project Atlas нашёл его, но не показывает неполную статистику объектов.', detailBoundary: 'Здесь показан результат последней локальной проверки, а не анализ ИИ.', artifactTypes: { file: 'Файлы', directory: 'Папки', document: 'Документы', source_code: 'Исходный код', configuration: 'Конфигурация', unknown: 'Без категории' }, noProjects: 'В этой папке не найдены программные проекты.', unknownTechnology: 'Технологии не определены', items: 'объектов', analysisLimited: 'Размер проекта превышает текущий объём проверки', assetCountUnavailable: 'Число объектов не определено', changeStatus: { added: 'Новый', changed: 'Изменён', unchanged: 'Без изменений', recorded: 'Записан' }, localBoundary: 'Пути к папкам, данные проектов и записи проверок остаются на этом устройстве. Автопроверки остановятся после закрытия локальной службы.' },
    executionPlan: 'Развитие проекта', architectureEvolution: 'Ход работ', phasesComplete: '10 из 10 этапов',
    phases: [
      ...
      { name: 'Сведения о проекте', summary: 'Просмотр данных проверки отдельного проекта.', features: ['Открытие по названию проекта', 'Статус, технологии и категории объектов', 'Ясная отметка ограниченных проектов без выдуманной статистики'] },
    ...
    milestones: [['Доступные по нажатию сведения о проекте', 'Статус проверки, технологии и категории объектов из результатов поиска'], ['Большие проекты не останавливают поиск', 'Проекты за пределами проверки отмечаются как ограниченные, остальные продолжают обрабатываться'], ['Управление локальными папками проектов', 'Выбор папок, поиск проектов и плановая проверка изменений'], ['Ясный состав проекта и записи релизов', 'Состав, связи проектов и анализ в реальном времени разделены']],
  ...
  ko: {
    ...
    localOnly: '로컬 전용', dataStays: '데이터는 이 기기에만 저장됩니다', workspace: '프로젝트 작업 공간', greeting: '프로젝트 개요', planComplete: '프로젝트 계획 1.6 · 완료',
    ...
    workspaceManager: { section: '프로젝트 탐색', title: '관리할 폴더 선택', description: '로컬 폴더를 선택하면 Project Atlas가 그 안의 소프트웨어 프로젝트를 식별합니다. 자동 점검은 사용자가 직접 켜야 시작됩니다.', chooseDirectory: '폴더 선택', selecting: '선택 대기 중…', connecting: '로컬 서비스에 연결 중…', serviceUnavailable: '로컬 서비스가 실행 중이 아닙니다', serviceHelp: 'Project Atlas 로컬 서비스를 시작한 다음 다시 연결하세요.', retry: '다시 연결', emptyTitle: '추가된 프로젝트 폴더가 없습니다', emptyDescription: '폴더를 선택하면 첫 프로젝트 탐색이 즉시 실행됩니다.', initialScanComplete: '폴더가 추가되었고 첫 프로젝트 탐색이 완료되었습니다.', scanComplete: '프로젝트 점검이 완료되었습니다.', operationFailed: '작업을 완료하지 못했습니다.', scanDirectory: '스캔 폴더', lastChecked: '최근 점검', neverScanned: '아직 점검하지 않음', scanNow: '지금 점검', remove: '제거', confirmRemove: '이 폴더와 로컬 점검 기록을 제거할까요? 폴더의 파일은 삭제되지 않습니다.', automaticChecks: '자동 점검', interval: '점검 간격', minutes: '분', latestChanges: '최근 점검 결과', projectCount: '개 프로젝트', newProjects: '개 추가', changedProjects: '개 변경', removedProjects: '개 제거', limitedProjects: '개 제한', discoveredProjects: '식별된 프로젝트', openProject: '프로젝트 보기', projectDetail: '프로젝트 상세', closeDetail: '프로젝트 상세 닫기', projectPath: '프로젝트 경로', checkResult: '최근 점검 결과', structureCheck: '구조 점검', completeCheck: '완료', limitedCheck: '제한됨', technologyStack: '기술 스택', assetOverview: '프로젝트 자산 개요', limitedDetail: '이 프로젝트는 현재 구조 점검 한도를 초과합니다. Project Atlas는 프로젝트를 식별했지만 불완전한 자산 통계는 표시하지 않습니다.', detailBoundary: '최근 로컬 점검 결과이며 AI 분석이 아닙니다.', artifactTypes: { file: '일반 파일', directory: '폴더', document: '문서', source_code: '소스 코드', configuration: '설정 파일', unknown: '미분류' }, noProjects: '이 폴더에서 소프트웨어 프로젝트를 식별하지 못했습니다.', unknownTechnology: '기술 스택 미식별', items: '개 자산', analysisLimited: '프로젝트 규모가 현재 점검 범위를 초과함', assetCountUnavailable: '자산 수 미집계', changeStatus: { added: '추가됨', changed: '변경됨', unchanged: '변경 없음', recorded: '기록됨' }, localBoundary: '폴더 경로, 프로젝트 정보, 점검 기록은 이 기기에만 저장됩니다. 로컬 서비스를 종료하면 자동 점검도 멈춥니다.' },
    executionPlan: '프로젝트 개발', architectureEvolution: '개발 현황', phasesComplete: '10 / 10 단계',
    phases: [
      ...
      { name: '프로젝트 상세', summary: '탐색 결과에서 개별 프로젝트 점검 상세를 확인합니다.', features: ['프로젝트 이름을 눌러 상세 열기', '점검 상태, 기술 스택, 자산 분류 확인', '제한 프로젝트를 명확히 표시하고 통계를 꾸미지 않음'] },
    ...
    milestones: [['클릭 가능한 로컬 프로젝트 상세', '탐색 결과에서 점검 상태, 기술 스택, 자산 분류 확인'], ['대규모 프로젝트가 폴더 탐색을 중단하지 않음', '점검 범위를 넘는 프로젝트는 제한으로 표시하고 나머지는 계속 식별'], ['로컬 프로젝트 폴더 관리', '스캔 폴더를 선택하고 프로젝트를 식별하여 정기적으로 변경 점검'], ['명확한 프로젝트 구성과 릴리스 기록', '프로젝트 구성, 프로젝트 관계, 실시간 분석을 구분']],
