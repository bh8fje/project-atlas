...
当前仓库已完成 Execution Plan 1.6：除工程基础、项目理解、历史、知识、界面和多语言能力外，还可通过本机服务选择项目目录、识别其中的软件项目、定时检查结构变化，并点击项目查看最近一次本机检查详情。仓库仍不内置具体模型适配器、Git commit 分析、公网服务或外部 API。
...
点击已识别项目名称，可查看路径、最近检查状态、技术栈和资产分类。项目详情只展示已有本机检查结果，不重新扫描，也不是 AI 分析。
...
如果目录中的单个项目超过 10,000 项结构检查上限，系统会将该项目标记为受限并继续识别其他项目，不会让整个目录检查失败。
